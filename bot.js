...
async function notifyRestart() {
  ...
  🚀 *Бот перезапущен и снова в строю!*
    ...
    Обновление специально для Матвея, который не может проголосовать за фильмы из-за падающего трона
    `.trim();
...
bot.command("notifyForMatveyExtra", async (ctx) => {
  if (!(await isAdmin(ctx.from.id)))
    return ctx.reply("❌ У вас нет доступа к этой команде.");

  try {
    const message = "ЕБАНЫЙ РОТ ТВОЙ МАТВЕЙ, ТЫ ЧТО, БЛЯТЬ, СОВСЕМ ОХУЕЛ?";

    try {
      await ctx.telegram.sendMessage(2101418619, message);
    } catch (e) {
      console.warn(`Не удалось отправить сообщение пользователю ${2101418619}`);
    }
...
bot.command("notifyForMatvey", async (ctx) => {
  if (!(await isAdmin(ctx.from.id)))
    return ctx.reply("❌ У вас нет доступа к этой команде.");

  try {
    const message = "ЕБАНЫЙ РОТ ТВОЙ МАТВЕЙ, ТЕБЕ ОТДЕЛЬНОЕ ПРИГЛАШЕНИЕ НУЖНО? ПРОГОЛОСУЙ ТЫ ЗА ФИЛЬМЫ, ЭТО НЕ ЗАНИМАЕТ МНОГО ВРЕМЕНИ. Я ПОНИМАЮ, ЧТО ЖЕНА РОЖАЕТ, ТРОН ПАДАЕТ В ДОТЕ, НО УДЕЛИТЬ 2 МИНУТЫ МОЖНО";
