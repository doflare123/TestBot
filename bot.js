...
bot.command("notifyForMatvey", async (ctx) => {
  ...
  try {
    const message = "ЕБАНЫЙ РОТ ТВОЙ МАТВЕЙ, ТЕБЯ ОТДЕЛЬНОЕ ПРИГЛАШЕНИЕ НУЖНО? ПРОГОЛОСУЙ ТЫ ЗА ФИЛЬМЫ ЭТО НЕ ЗАНИМАЕТ МНОГО ВРЕМЕНИ. Я ПОНИМАЮ, ЧТО ЖЕНА РОЖАЕТ, ТРОН ПАДАЕТ В ДОТЕ, НО УДЕЛИТЬ 2 МИНУТЫ МОЖНО";
